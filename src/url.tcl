...
proc ::bonaPRE::urlADD { nick uhost hand chan arg } {
  ...
  set UAD_Chan      ${chan}
  ::bonaPRE::urlADDEXEC ${nick} ${UAD_Chan} ${UAD_DateTime} ${UAD_Tvmaze} ${UAD_Name} ${UAD_Group} ADDURL ${::bonaPRE::dburl_(addurl)}
  return false;
}
...
proc ::bonaPRE::urliMDB { nick uhost hand chan arg } {
  ...
  set UIM_Chan      ${chan}
  ::bonaPRE::urlADDEXEC ${nick} ${UIM_Chan} ${UIM_DateTime} ${UIM_Tvmaze} ${UIM_Name} ${UIM_Group} iMDB ${::bonaPRE::dburl_(imdb)}
  return false;
}
...
proc ::bonaPRE::urlTVMAZE { nick uhost hand chan arg } {
  ...
  set UTM_Chan      ${chan}
  ::bonaPRE::urlADDEXEC ${nick} ${UTM_Chan} ${UTM_DateTime} ${UTM_Tvmaze} ${UTM_Name} ${UTM_Group} TVMAZE ${::bonaPRE::dburl_(tvmaze)}
  return false;
}
...
proc ::bonaPRE::urlADDEXEC { args } {
  ...
  if { ${UAE_Var} == "" } {
    if { ${UAE_Sta} == "ADDURL" } {
      ...
      set UAE_MSGERR  [format "Syntax * %s <nom.de.la.release> <lien-url>" ${::bonaPRE::CMD(addurl)}]
      putquick "privmsg ${UAE_Chan} ${UAE_MSGERR}"
      return -code error ${UAE_LOGERR};
    }
    if { ${UAE_Sta} == "iMDB" } {
