...
proc ::bonaPRE::ainfo { nickSource hostSource hand channelSource arg } {
  ...
  set I_Chan        ${channelSource}
  ::bonaPRE::addinfo ${nickSource} ${I_Chan} ${I_DateTime} ${I_Name} ${I_Files} ${I_Size} INFO
  return false;
}

proc ::bonaPRE::oinfo { nickSource hostSource hand channelSource arg } {
  ...
  set IO_Chan       ${channelSource}
  ::bonaPRE::addinfo ${nickSource} ${IO_Chan} ${IO_DateTime} ${IO_Name} ${IO_Files} ${IO_Size} OLDINFO
  return false;
}
...
proc ::bonaPRE::addinfo { args } {
  ...
  append AI_SqlUP   "WHERE `${::bonaPRE::db_(releaseName)}`='${AI_Rls}';";
  set AI_SqldUP     [::bonaPRE::MySQL::exec ${AI_SqlUP}];
  set AI_Sqldid1    "SELECT `${::bonaPRE::db_(id)}` FROM `${::bonaPRE::mysql_(dbmain)}` WHERE `${::bonaPRE::db_(releaseName)}` LIKE '${AI_Rls}%'";
